...
module Jekyll
  class Explore < Liquid::Block
    ...
    def initialize(tag_name, markup, tokens)
      ...
      if markup.strip =~ SYNTAX
        ...
        raise SyntaxError, <<~MSG
            Syntax Error in tag 'explore' while parsing the following markup:
            #{markup}
    ...
    def add_code_tag(code)
      url = @highlight_options[:url]
      code_attrs = %(class="language-#{@lang.tr("+", "-")}" data-lang="#{@lang}")
      %(<div><figure class="highlight"><pre><code #{code_attrs}>#{code.chomp}</code></pre><div class="example-play-button"><a href=#{url} target="_blank" rel="noopener noreferrer"><i class="fa-play"></i></a></div></figure></div>)
    end
  end
end

Liquid::Template.register_tag("explore", Jekyll::Explore)
